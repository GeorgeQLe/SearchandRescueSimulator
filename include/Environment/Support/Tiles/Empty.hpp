...
namespace nsEmpty {
    class Empty : public nsTile::Tile {
        public:
        // 
        Empty(const nsTerrainType::TerrainType new_tile_type, unsigned x, unsigned y) : nsTile::Tile(x, y, new_tile_type) {
            // set_symbol is a member function of the parent Tile
            // that changes the symbol that is printed during the
            // simulation
            this->set_symbol(' ');
        }
        Empty(const nsCoord::Coord& coordinates, const nsTerrainType::TerrainType new_tile_type) : nsTile::Tile(coordinates, new_tile_type) {
            // set_symbol is a member function of the parent Tile
            // that changes the symbol that is printed during the
            // simulation
            this->set_symbol(' ');
